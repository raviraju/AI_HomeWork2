...
bool distributeStonesP1(int playerTurn,int& pitValue, int i, int j, int** board, int noOfPits, map<pair<int,int>, string >& positionMancalaMap);
bool distributeStonesP2(int playerTurn,int& pitValue, int i, int j, int** board, int noOfPits, map<pair<int,int>, string >& positionMancalaMap);
...
bool distributeStonesP1(int playerTurn,int& pitValue, int i, int j, int** board, int noOfPits, map<pair<int,int>, string >& positionMancalaMap)
{
	while( (j <= noOfPits+1) && (pitValue > 0) )
	{
		...
		{
			if((pitValue == 0)&&(playerTurn==1))//last stone added to mancala
				return true;
		}
	...
	if(pitValue > 0)//remaining stones add to other player's pits
		return distributeStonesP2(playerTurn, pitValue,0,noOfPits,board,noOfPits, positionMancalaMap);
	return false;
}
...
bool distributeStonesP2(int playerTurn,int& pitValue, int i, int j, int** board, int noOfPits, map<pair<int,int>, string >& positionMancalaMap)
{
	while( (j >= 0) && (pitValue > 0) )
	{
		...
		if(it != positionMancalaMap.end())
		{
			if((pitValue == 0)&&(playerTurn==2))//last stone added to mancala
				return true;
		}
		j = j-1;//move to next pit
	}
	if(pitValue > 0)
		return distributeStonesP1(playerTurn, pitValue,1,1,board,noOfPits, positionMancalaMap);
	return false;
}

bool pickPit(int playerTurn,string pitName, int noOfPits, int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap)
{
	...
	if(i==1)//Player1
	{
		return distributeStonesP1(playerTurn,pitValue,i,j+1,board,noOfPits,positionMancalaMap);
	}
	if(i==0)//Player2
	{
		return distributeStonesP2(playerTurn,pitValue,i,j-1,board,noOfPits,positionMancalaMap);
	}
	return false;
}
	...
	while( !checkGameOver(noOfPits, board, pitNamePositionMap) )
	{
		if(player1Turn)
		{
			...
			if(validMove(move,1,playerValidMovesMap))
			{
				...
				anotherChance = pickPit(1,move, noOfPits, board, pitNamePositionMap, positionMancalaMap);
				cout << "After " << move << " : ";
				displayBoard(noOfPits, board, playerValidMovesMap);
		...
		else
		{
			...
			{
				bool anotherChance=false;
				anotherChance = pickPit(2,move, noOfPits, board, pitNamePositionMap, positionMancalaMap);
				cout << "After " << move << " : ";
				displayBoard(noOfPits, board, playerValidMovesMap);
				if(anotherChance == true)
				{

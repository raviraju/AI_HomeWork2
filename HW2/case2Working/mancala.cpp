...
int H_MinValue(ofstream& ofs,string action,bool nodeType,bool haveAnotherChance,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
int H_MaxValue(ofstream& ofs,string action,bool nodeType,bool haveAnotherChance,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
...
void H_MiniMax(ofstream& ofs,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo)
{
	...
	for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
	{
		...
		if(oneMoreChance)
		{
			val = H_MaxValue(ofs,pitName,maxNodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,playerNo,curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
		}
		else
		{
			val = H_MinValue(ofs,pitName,!maxNodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
		}
...
int H_MinValue(ofstream& ofs,string nodeName,bool nodeType,bool haveAnotherChance,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap)
{
	for(int d=0;d<curDepth;d++)
		cout << "\t";
	...
	if(curDepth == cutOfDepth)
	{
		for(int d=0;d<curDepth;d++)
			cout << "\t";
		cout << "cutOfDepth : " << cutOfDepth << " reached hence computing EVAL of board " << endl;
		nodeValue = Eval(board,noOfPits,playerNo);
		write_TraverseLog(ofs,nodeName,curDepth,nodeValue);
	...
	if(it != playerValidMovesMap.end())
	{
		...
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			if(oneMoreChance)
			{
				val = H_MinValue(ofs,pitName,nodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,playerNo,depth,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			}
			else
			{
				val = H_MaxValue(ofs,pitName,!nodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),depth,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			}
			if(val <= nodeValue)
			{
				nodeValue = val;
...
int H_MaxValue(ofstream& ofs,string nodeName,bool nodeType,bool haveAnotherChance,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap)
{
	for(int d=0;d<curDepth;d++)
		cout << "\t";
	...
	if(it != playerValidMovesMap.end())
	{
		...
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			if(oneMoreChance)
			{
				val = H_MaxValue(ofs,pitName,nodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,playerNo,depth,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			}
			else
			{
				val = H_MinValue(ofs,pitName,!nodeType,oneMoreChance,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),depth,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			}
			if(val >= nodeValue)
			{
				nodeValue = val;

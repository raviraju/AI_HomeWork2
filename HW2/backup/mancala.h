...
int H_MinValue(int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
int H_MaxValue(int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
...
bool result(int playerTurn,string pitName, int noOfPits, int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap, map<int, set<string> >& playerValidMovesMap)
{
...
string H_MiniMax(int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo,int curDepth)
{
	...
	if(it != playerValidMovesMap.end())
	{
		...
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			bool oneMoreChance = result(playerNo,*sItr, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			if(oneMoreChance)
				H_MiniMax(tempBoard,pitNamePositionMap, positionMancalaMap, playerValidMovesMap,noOfPits,cutOfDepth,playerNo,curDepth);

			int val = H_MinValue(tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
...
int H_MinValue(int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap)
{
	...
	if(curDepth == cutOfDepth)
	{
		for(int d=0;d<curDepth;d++)
			cout << "\t";
		cout << "cutOfDepth : " << cutOfDepth << " reached hence returing EVAL of board " << endl;
		return Eval(board,noOfPits,playerNo);
	}
	...
	if(it != playerValidMovesMap.end())
	{
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			for(int i = 0; i < 2; ++i)
				tempBoard[i] = new int[noOfPits+2];
			copy(board,tempBoard,noOfPits);
			result(playerNo,*sItr, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			int val = H_MaxValue(tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);

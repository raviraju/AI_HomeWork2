...
int H_MinValue(ofstream& ofs,string action,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
int H_MaxValue(ofstream& ofs,string action,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap);
...
int H_MiniMax(ofstream& ofs,string parent,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo,int curDepth)
{
	cout << "My Parent move is : " << parent;
	string action="NULL";
	...
	if(it != playerValidMovesMap.end())
	{
		...
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			pitNamePositionMapItr it = pitNamePositionMap.find(*sItr);
			if(it == pitNamePositionMap.end())
			{
				cout << "PitName : " << *sItr << " not found!!!,skipping..." << endl;
				continue;
			}
			int i = (it->second).first;
			int j = (it->second).second;
			...
			if (pitValue == 0)
			{
				cout << "PitName : " << *sItr << " has : " << pitValue << " stones " << endl;
				cout << "Hence skipping..." << endl;
				continue;
			}
			cout << "MAX(Player : " << playerNo << " ) chose pit " << *sItr << endl;
			for(int d=0;d<curDepth;d++)
					cout << "\t";
			cout << *sItr << "," << curDepth+1 << "," << 9999999 << endl;
			ofs << *sItr << "," << curDepth+1 << "," << 9999999 << endl;
			string luckyMove = *sItr;
			bool oneMoreChance = result(curDepth,playerNo,*sItr, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			if(oneMoreChance)
			{
				int v = H_MiniMax(ofs,*sItr,tempBoard,pitNamePositionMap, positionMancalaMap, playerValidMovesMap,noOfPits,cutOfDepth,playerNo,curDepth);
				maxEval = v;
			}
			else
			{
				int val = H_MinValue(ofs,*sItr,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
		...
		for(int d=0;d<curDepth;d++)
			cout << "\t";
		cout << parent << "," << curDepth+1 << "," << maxEval << "//parent"<< endl;
		ofs << parent << "," << curDepth+1 << "," << maxEval << "//parent"<< endl;
...
int H_MinValue(ofstream& ofs,string action,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap)
{
	...
	if(curDepth == cutOfDepth)
	{
		for(int d=0;d<curDepth;d++)
			cout << "\t";
		cout << "cutOfDepth : " << cutOfDepth << " reached hence returning EVAL of board " << endl;
		int eval = Eval(board,noOfPits,playerNo);
		for(int d=0;d<curDepth;d++)
			cout << "\t";
		cout << action << "," << curDepth << "," << eval << endl;
		ofs << action << "," << curDepth << "," << eval << endl;
		return eval;
	}

	int min_of_MaxValues = 9999999;
	...
	if(it != playerValidMovesMap.end())
	{
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			for(int i = 0; i < 2; ++i)
				tempBoard[i] = new int[noOfPits+2];
			copy(board,tempBoard,noOfPits);
			result(curDepth,playerNo,*sItr, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			int val = H_MaxValue(ofs,*sItr,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			if(val <= min_of_MaxValues)
			{
				min_of_MaxValues = val;
				for(int d=0;d<curDepth;d++)
					cout << "\t";
				cout << action << "," << curDepth << "," << min_of_MaxValues << endl;
				ofs << action << "," << curDepth << "," << min_of_MaxValues << endl;
			}
		}
	}
	return min_of_MaxValues;
}
...
int H_MaxValue(ofstream& ofs,string action,int** board,int noOfPits,int cutOfDepth,int playerNo,int curDepth,map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap)
{
	...
	int max_of_MinValues = -9999999;
	for(int d=0;d<curDepth;d++)
		cout << "\t";
	cout << action << "," << curDepth << "," << max_of_MinValues << endl;
	playerValidMovesMapItr it;
	it = playerValidMovesMap.find(playerNo);
	if(it != playerValidMovesMap.end())
	{
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			result(curDepth,playerNo,*sItr, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			int val = H_MinValue(ofs,*sItr,tempBoard,noOfPits,cutOfDepth,opponent(playerNo),curDepth+1,pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			if(val >= max_of_MinValues)
			{
				max_of_MinValues = val;
				for(int d=0;d<curDepth;d++)
					cout << "\t";
				cout << action << "," << curDepth << "," << max_of_MinValues << "//max_of_MinValues:521" << endl;
				ofs << action << "," << curDepth << "," << max_of_MinValues << "//max_of_MinValues:521" << endl;
			}
		}
	}
	return max_of_MinValues;
}
...
int main(int argc, char *argv[])
{
	...
	H_MiniMax(ofs_traverse_log,"root",board,pitNamePositionMap, positionMancalaMap,playerValidMovesMap,noOfPits,cutOfDepth,playerNo,0);
	return 0;

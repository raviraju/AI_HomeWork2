...
string H_MiniMax(ofstream& ofs,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo)
{
...
void MiniMax_Algo(ofstream& ofs_next_state,ofstream& ofs_traverse_log,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo)
{
	...
	int maxValue = Eval(nextBoard,noOfPits,playerNo);
	int** maxBoard = new int*[2];
	for(int i = 0; i < 2; ++i)
		maxBoard[i] = new int[noOfPits+2];
	copy(nextBoard,maxBoard,noOfPits);
	
	while(anotherChanceExists)
	{
		displayValidMoves(playerNo,playerValidMovesMap);
		playerValidMovesMapItr it;
		it = playerValidMovesMap.find(playerNo);
		...
		for(setStringsItr sItr = (it->second).begin(); sItr != (it->second).end(); sItr++)
		{
			...
			for(int i = 0; i < 2; ++i)
				tempBoard[i] = new int[noOfPits+2];
			copy(nextBoard,tempBoard,noOfPits);
			
			anotherChanceExists = result(0,playerNo,pitName, noOfPits, tempBoard, pitNamePositionMap, positionMancalaMap, playerValidMovesMap);
			int val = Eval(tempBoard,noOfPits,playerNo);
			cout << "Gain for the move : " << val << endl;
			if(val > maxValue)//consider that move
			{
				maxValue = val;
				cout << "Updating max Gain : " << maxValue << endl;
				copy(tempBoard,maxBoard,noOfPits);
			}
			cout << "*************************************************************************" << endl;
		} 
	}
	
	cout << "Max Value for next state using H_MiniMax : " << maxValue << endl;
	cout << "Next State:" << endl;
	displayBoard(0,noOfPits, maxBoard, playerValidMovesMap);
	
	//board state for player-2
	int i = 0;
	for(int j =1; j<(noOfPits+2); j++)
		if(maxBoard[i][j] != -1)
			ofs_next_state << maxBoard[i][j] << " ";
	ofs_next_state << endl;
	...
	ofs_next_state << endl;
	//player-2’s mancala
	ofs_next_state << maxBoard[0][0] << endl;
	//player-1’s mancala
	ofs_next_state << maxBoard[1][noOfPits+1] << endl;
}

string AB_H_MiniMax(int alpha,int beta,ofstream& ofs,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo)
{
	bool maxNodeType = true;//As Root is always Max Node Type
	int curDepth = 0;
	//int maxEval = defaultNodeValue(maxNodeType);
...
void AlphaBeta_Algo(ofstream& ofs_next_state,ofstream& ofs_traverse_log,int** board, map<string,pair<int,int> >& pitNamePositionMap, map<pair<int,int>, string >& positionMancalaMap,map<int, set<string> >& playerValidMovesMap, int noOfPits,int cutOfDepth,int playerNo)
{
	...
	cout << "Max Value for next state  : " << maxValue << endl;
	cout << "Next State:" << endl;
	displayBoard(0,noOfPits, maxBoard, playerValidMovesMap);
